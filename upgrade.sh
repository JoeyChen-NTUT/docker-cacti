...
wget -qO- --no-check-certificate $cacti_download_url | tar xzC /tmp/update/cacti

# download and uncompress spine
echo "$(date +%F_%R) [Upgrade] Downloading Spine from $spine_download_url"
wget -qO- --no-check-certificate /tmp/update $spine_download_url | tar xzC /tmp/update/spine
